...
void LCD_GPIO_init(void)
{
    ...
    GPIO_Init(LCD_PORT6, LCD_DB6, GPIO_MODE_OUT_PP_HIGH_FAST);
...
{                                     
    ...
    GPIO_WriteLow(LCD_PORT6, LCD_DB6);   
...
{
    bool temp = FALSE;
    GPIO_TypeDef *LPort = (pin_num==LCD_DB6) ? LCD_PORT6:LCD_PORT;
    ...
    switch(temp)
    {
        case TRUE:
        {
            GPIO_WriteHigh(LPort, pin_num);   
            break;
        }
 
        default:
        {
            GPIO_WriteLow(LPort, pin_num);   
            break;
         }
      }
}